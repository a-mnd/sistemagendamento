...
<body>
    ...
    ?>
    <?php
    // Caso o usuário não esteka logado na sessão e não tenha o perfil verificado
    if (!isset($_SESSION['email']) || !isset($_SESSION['contato']) || $_SESSION['validado'] == 0) {
        include_once(__DIR__ . '/../admin/telaValidarEmail.php');
    } else { // Caso o usuário tenha feito o login e o usuário verificado
    ?>
                    ...
                    <tbody>
                        <?php
                                if (isset($_POST['mes'])) {
                                    ...
                                    // SQL PARA USUARIO
                                    $sql2 = "SELECT a.id_agendamento, a.dia, s.nome_servico, a.hora, f.nome_funcionario, a.nome_cliente FROM agendamento a JOIN servicos s ON a.id_servico = s.id_servico JOIN funcionario f ON a.id_funcionario = f.id_funcionario WHERE DATE_FORMAT(a.dia, '%Y-%m') = :ano_mes AND a.email = :email_cliente AND a.contato = :contato_cliente";
                                    $prep2 = $conexao->prepare($sql2);
                                    $prep2->execute(['ano_mes' => "$ano-$mes", 'email_cliente' => $email_cliente, 'contato_cliente' => $contato_cliente]);
                        
                        
                                    if ($prep2->rowCount() > 0) {
                                        while ($array = $prep2->fetch(PDO::FETCH_ASSOC)) {
